...
export const AsidePainel = async ({ className }: AsidePainelProps) => {
  ...
  const todayBookings = barbershops.flatMap((barbershop) =>
    barbershop.bookings.filter((booking) => {
  ...
  const myBarbershop = barbershops.filter(
    (barbershops) => barbershops.user.id === (session?.user as any).id,
  );
