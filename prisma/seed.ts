...
async function seedDatabase() {
	...
	try {
		...
		const services = [
          ...
          'https://utfs.io/f/c4919193-a675-4c47-9f21-ebd86d1c8e6a-4oen2a.png',
			},
			{
				...
				price: 25.0,
				imageUrl:
          'https://utfs.io/f/6cf53916-0096-4d31-ac5e-1b504efad89c-28jn.png',
			},
		];
		...
		for (let i = 0; i < 10; i++) {
			...
			// Gerar DDD e número fictício
			const ddd = (11 + i).toString().padStart(2, '0');
			const numBase = (90000000 + i).toString();
			const phoneFormatted = `(0${ddd}) ${numBase.slice(0,4)} - ${numBase.slice(4)}`;
			const phoneFormatted2 = `(0${ddd}) ${(numBase + '1').slice(0,4)} - ${(numBase + '1').slice(4,8)}`;
			...
			const barbershop = await prisma.barbershop.create({
				data: {
					...
					name: name,
					userId: user.id,
					phone1: phoneFormatted,
					phone2: phoneFormatted2,
				},
			});
